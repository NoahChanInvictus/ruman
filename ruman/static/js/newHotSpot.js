// 热点监测 页面 js 【综合的微博热点、新闻热点。 现在是复制的 新闻热点（溯源分析）的】【点击进入详情 时 分 微博详情和新闻热点详情】
...
// 热点监测 表格
    ...
    // var hotSpot_url = '/hotSpot/hotspotNewsText';
    var hotSpot_url = '/newHotSpot/newhotspotandrumanText/';
        ...
        $('#hotspotTable').bootstrapTable({
            ...
            columns: [
                ...
                },
                {
                    ...
                    valign: "middle",//垂直
                    formatter: function (value, row, index) {
                        if (row.publisher==''||row.publisher=='null' || row.publisher==null ||row.publisher=='unknown'||!row.publisher){
                            return '未知';
                        }else {
                ...
                },
                {
                    ...
                    valign: "middle",//垂直
                    formatter: function (value, row, index) {
                        if (row.publish_time==''||row.publish_time=='null' || row.publish_time==null ||row.publish_time=='unknown'||!row.publish_time){
                            return '未知';
                        }else {
                ...
                },
                {
                    ...
                    valign: "middle",//垂直
                    formatter: function (value, row, index) {
                        if (row.source==''||row.source=='null' || row.source==null ||row.source=='unknown'||!row.source){
                            return '未知';
                        }else {
                            return row.source;
                        };
                ...
                },
                {
                    ...
                    valign: "middle",//垂直
                    formatter: function (value, row, index) {
                        // if (row.keyword==''||row.keyword=='null' || row.keyword==null ||row.keyword=='unknown'||!row.keyword){
                        //     return '未知';
                        // }else {
                        //     return row.keyword;
                        // };
                        ...
                        if (row.keyword==''||row.keyword=='null'||row.keyword=='unknown'||!row.keyword){
                            return '未知';
                        }else if(row.keyword.length >=20){
                            keyword = row.keyword.slice(0,20)+'...';
                            return '<span style="cursor:pointer;color:white;" title="'+row.keyword+'">'+keyword+'</span>';
                        }else {
                            keyword = row.keyword;
                            return '<span style="cursor:pointer;color:white;" title="'+row.keyword+'">'+keyword+'</span>';
                        };
